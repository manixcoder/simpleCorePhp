...
if (isset($_REQUEST['user_id']) && !empty($_REQUEST['user_id']))
	...
	if ($res != 0)
		...
		if ($res1 != 0)
			...
			$sql = "SELECT r. * , pc. * FROM feeds_m pc, registration r WHERE pc.feedowner_id = '$user_id' 
                        AND pc.action_owner_id = r.user_id ORDER BY `date` DESC  ";
			...
			if ($res > 0)
				{
				$result = array("response" => array('code' => '201','message' => "Feed found successfully",'data' => $all_data));
			...
			$result = array("response" => array('code' => '200','message' => "There is no feeds",'data' => $r1));
			print_r(json_encode($result));
			}
		}
	  else
