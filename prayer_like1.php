...
if (isset($_REQUEST['prayer_id']) && !empty($_REQUEST['prayer_id']) && isset($_REQUEST['user_id']) && !empty($_REQUEST['user_id']))
	...
	if ($res == 0)
		...
		if ($res1 !=0 )
			...
			$result = array("response" => array('code' => '201','message' => "You have liked successfully.",'like' => $likes1));
			print_r(json_encode($result));
					...
					if ($feedowner_id == $user_id)
						...
						//$sql = "SELECT * FROM `feeds_m` WHERE `post_id` ='$p_id' AND `feed_type`='$hh' OR `feed_type`='$gg'";
						$sql = "SELECT * FROM `feeds_m` WHERE `post_id` ='$p_id' AND `feed_type`='$hh' OR `feed_type`='$gg'";
			...
			//echo $sql = "SELECT * FROM `feeds_m` WHERE `post_id` ='$p_id'";
...
//Push notification end
		...
		if ($result)
			...
			$result = array("response" => array('code' => '201','message' => "You have dislike the Prayer",'like' => $li));
			print_r(json_encode($result));
